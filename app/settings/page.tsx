...
export default function Settings() {
  ...
  if (!me) return <main className="min-h-dvh grid place-items-center"><div className="skeleton h-40 w-72" /></main>;
  ...
  return (
    <main className="min-h-dvh px-5 py-10 max-w-lg mx-auto space-y-6">
      ...
      <section className="glass p-6">
        ...
        <p className="text-sm text-mist">{me.firstName} · {me.email}</p>
        <button onClick={logout} className="btn-ghost w-full mt-4 text-sm">Log out</button>
      </section>
      ...
      <section className="glass p-6 space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          {(["ACTIVE", "PAUSED", "HIDDEN"] as const).map((s) => (
            <button key={s} className="chip" data-on={me.status === s} onClick={() => setStatus(s)}>
              {s === "ACTIVE" ? "Active" : s === "PAUSED" ? "Paused" : "Hidden"}
            </button>
          ))}
        </div>
      ...
      <section className="glass p-6 space-y-3">
        ...
        {!confirmDelete ? (
          <button onClick={() => setConfirmDelete(true)} className="w-full text-sm text-danger py-3">
            Delete my account permanently
          </button>
        ) : (
          <div className="border border-danger/40 rounded-card p-4 space-y-3">
            <p className="text-sm">
              This removes your profile, matches, and messages. It cannot be undone.
            </p>
            <div className="flex gap-2">
              ...
              <button onClick={() => setConfirmDelete(false)} className="btn-ghost flex-1 text-sm">
                Keep my account
              </button>
            </div>
          </div>
