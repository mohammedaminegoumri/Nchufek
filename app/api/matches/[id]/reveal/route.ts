  ...
  try {
    ...
    if (body.data.action === "request") {
      ...
      const otherRequested =
        (match.reveal === "REQUESTED_BY_A" &&
          match.userAId === otherId) ||
        (match.reveal === "REQUESTED_BY_B" &&
          match.userBId === otherId);

      const newState = otherRequested
        ? "MUTUAL"
    ...
    const updated = await prisma.match.update({
      where: { id: match.id },
      data: {
        reveal: body.data.accept ? "MUTUAL" : "DECLINED",
      },
    });
